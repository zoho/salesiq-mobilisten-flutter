...
@objc(MobilistenCallsPlugin)
public class SwiftMobilistenCallsPlugin: NSObject, FlutterPlugin, FlutterSceneLifeCycleDelegate {
    ...
    // Shared fallback error codes for Calls. Negative (wrapper-synthetic, not native SalesIQ codes,
    // which are positive) and kept in sync with the Android Calls plugin (MobilistenCallsPlugin):
    // -2000 invalid VoIP token (iOS), -2001 view init failed (Android). Native errors keep their own
    // code via getCallError().
    private enum CallsError {
        static let invalidVoipToken = "-2000"
    }
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        Task { @MainActor in
            ...
            case "enableVoIP":
                enableVoIP(argument: call.argumentDictionary, result: result)
            case "handleVOIPNotificationAction":
                handleVOIPNotificationAction(argument: call.arguments as? [AnyHashable: Any], result: result)
    ...
    // MARK: - enableVoIP
    @MainActor private func enableVoIP(argument: [String: Any]?, result: @escaping FlutterResult) {
        guard let token = argument?["token"] as? String, !token.isEmpty else {
            result(FlutterError(code: CallsError.invalidVoipToken, message: "Invalid VoIP token", details: nil))
            return
        }
        let isTestDevice = argument?["isTestDevice"] as? Bool ?? false
        let apnsMode: APNSMode = (argument?["mode"] as? String == "sandbox") ? .sandbox : .production
        ZohoSalesIQCalls.enableVoIP(token, isTestDevice: isTestDevice, mode: apnsMode)
        result(nil)
    }

    // MARK: - handleVOIPNotificationAction
    @MainActor private func handleVOIPNotificationAction(argument: [AnyHashable: Any]?, result: @escaping FlutterResult) {
        ZohoSalesIQCalls.handleVOIPNotificationAction(argument) {
            result(nil)
        }
    }
            ...
            if let departments = attributes["departments"] as? [[String: Any]] {
                ...
                departments.forEach { department in
                    ...
                    if id?.isEmpty == false || name?.isEmpty == false {
                        if let mode = department["communicationMode"] as? String, let communicationMode = getCommunicationMode(mode) {
    ...
    @MainActor private func setVisibility(argument: [String: Any]?, result: @escaping FlutterResult) {
        guard let component = argument?["component"] as? String else { return }
        let visible = argument?["isVisible"] as? Bool ?? true
